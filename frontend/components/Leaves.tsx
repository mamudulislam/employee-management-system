...
const Leaves: React.FC = () => {
    ...
    const [formData, setFormData] = useState<LeaveRequest>({
        employee: '',
        employeeName: AuthService.getUser()?.name || '',
        leaveType: 'Casual',
        startDate: '',
        endDate: '',
        reason: ''
    ...
    const calculateAvailableBalance = () => {
        ...
        const userApprovedLeaves = leaves.filter(leave => {
            const employeeObj = typeof leave.employee === 'object' ? leave.employee : null;
            const employeeId = employeeObj?._id || (typeof leave.employee === 'string' ? leave.employee : null);
            const employeeEmail = employeeObj?.email;

            // Match against current user's ID or email
            const isMatch = (employeeId && employeeId.toString() === currentUserId.toString()) ||
                (employeeEmail && employeeEmail === currentUser.email);
    ...
    const getEmployeeDisplayName = (leave: Leave) => {
        if (leave.employee && typeof leave.employee === 'object' && 'name' in leave.employee) {
            return leave.employee.name;
        }
        if (typeof leave.employee === 'string') {
            return `ID: ${leave.employee.substring(leave.employee.length - 6)}`;
        }
