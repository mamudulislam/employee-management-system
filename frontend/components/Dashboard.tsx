...
const LeaveRequestDialog: React.FC<{ isOpen: boolean; onClose: () => void; user: User }> = ({ isOpen, onClose, user }) => {
...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      try {
        ...
        console.log('Dashboard leaves data:', leavesData);
        // Get only first 5 leaves for dashboard
  ...
  return (
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              {pieData.map((d, i) => (
                <div key={i} className="flex flex-col p-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                  ...
                  <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{((d.value / 100234) * 100).toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </CardContent>
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-800 dark:text-slate-200 shadow-sm border border-white dark:border-slate-700">
                      {leave.employee?.name?.[0] || 
                       (typeof leave.employee === 'string' ? leave.employee.toString().substring(leave.employee.toString().length - 1).toUpperCase() : '?')}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-bold text-slate-900 dark:text-slate-100">
                            {leave.employee?.name || 
                             (typeof leave.employee === 'string' ? `Employee ${leave.employee.toString().substring(leave.employee.toString().length - 6)}` : 'Unknown Employee')}
                          </p>
    ...
    <LeaveRequestDialog isOpen={showLeaveRequest} onClose={() => setShowLeaveRequest(false)} user={user} />
    </>
  );
};
