...
import { Search, Filter, Plus, MoreVertical, FileDown, Upload, Trash2, Mail, Users as UsersIcon, X } from 'lucide-react';
...
const EmployeeDirectory: React.FC = () => {
  ...
  const loadEmployees = async () => {
    try {
      ...
      const errorMessage = err.message || err.error || 'Failed to load employees';
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedEmployees.size} selected employees?`)) {
      try {
        setLoading(true);
        const idsToDelete = Array.from(selectedEmployees);
        await EmployeeService.deleteBulkEmployees(idsToDelete);
        setSelectedEmployees(new Set());
        await loadEmployees();
        showToast.success('Employees deleted successfully');
      } catch (err: any) {
        showToast.error(err.message || 'Failed to delete employees');
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border-b border-indigo-100 dark:border-indigo-900/30 flex items-center justify-between animate-in slide-in-from-top duration-300">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="h-9 gap-2 rounded-xl text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:text-slate-400"
            >
              <X size={16} />
